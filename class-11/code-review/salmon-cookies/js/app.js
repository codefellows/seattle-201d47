 ...
 //Vinicio - normally, we try to avoid having global variables.
 //          global variables are prone to unexpected side effects
 var refTable = document.getElementById('cookies-stand');
 
 function getRandomNumber(min, max){ 
     // Vinicio - I basically console, logged before and after
    min = Math.ceil(min);
    max = Math.floor(max);
    
    // At some point, I realized this was the line that was breaking
    return Math.floor(Math.random() * (max - min + 1)) + min; //The maximum is inclusive and the minimum is inclusive 
 }

// Vinicio - smells - consider a better name
var finalTotal = 0;
var locations = [];
// Vinicio - this doesn't really smells that much, but... it does smell
...
function CookieStore(minCustomer, maxCustomer, avgCookieSale, id){
    ...
    // In programming, this is called a side effect
    locations.push(this); // Vinicio - I like and I don't like this line

    this.randomCookiesPerHour = function(){ // Then, I tried to find here
    ...
    // Vinicio - add more white space before or after the for loop
    this.render = function(){
        ...
        for(var i =0; i < hours.length; i++){
            var consumption = this.cookieSold(); // Check here first
            sum += consumption;
            this.consumptionArray.push(consumption);
            tdElement = document.createElement('td');
            tdElement.textContent = this.consumptionArray[i];
            trElement.append(tdElement); 
        } // Vinicio - you could (or could not) have a new line here
