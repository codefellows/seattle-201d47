...
function loadValuesFromLocalStorage() {
 ...
 if(votes) {
   // Vinicio - over here we could use the votes in whichever fashion we want
   var votesArray = JSON.parse(votes);
   // VInicio - I can only run this line AFTER I have values in allProductArray
   for(var i = 0; i < allProductArray.length; i++) {
     allProductArray[i].choosenProduct = votesArray[i];
   }
 }

 var clickCounter = localStorage.getItem('clickCounter');
 if(clickCounter) {
   globalStats.clickCounter = JSON.parse(clickCounter);
   // If you are doing a lot of work right here, you are not saving what you need to save
...
function Product(imagePath, descripition) {
    ...
    this.choosenProduct = 0; // This value is being stored in local storage?
...
var productHandler = function(event) {
    ...
    globalStats.clickCounter++;
    if (globalStats.clickCounter === 25) {
    ...
    function renderProgress() {
        ...
        if ( globalStats.clickCounter < 25) {
            progressMeter.textContent = `${globalStats.clickCounter}/25 chosen`
        }
        else {
...
var dataToStorage = function () {
    ...
    localStorage.setItem('clickCounter',globalStats.clickCounter);
};
